    ...
    ReentrancyGuardUpgradeable
{
    ...
    */
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 totalReward;
        if (oldReward[_rewardAddress] > rewards) {
            totalReward = 0;
        } else {
            totalReward =
    ...
    */
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = balancesLength - 1; i < balancesLength; i++) {
            if (amount <= stakedBalances[msg.sender][i]) {
                stakedBalances[msg.sender][i] -= amount;
                break;
            } else {
                amount = amount - stakedBalances[msg.sender][i];
