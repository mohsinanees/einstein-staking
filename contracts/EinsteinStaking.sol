    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public totalStakedAmount; // Total amount of tokens that users have staked
        ...
        whenNotPaused
    {
        ...
        if (transferStatus) {
            ...
            totalStakedAmount += _amount; // update Contract Staking balance
    ...
    */
        ...
        whenNotPaused
    {
        ...
        require(
            ...
            "STAKING: Requested balance must be less than or equal to Staked balance"
        );
    ...
    function _unStakeToken() internal virtual whenNotPaused {
        ...
        for (uint256 i = 0; i < stakedBalances[msg.sender].length; i++) {
            stakedBalances[msg.sender][i] = 0; // remove user staking balance
            stakingTimestamps[msg.sender][i] = 0; // remove user staking start times
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (balances > 0) {
            ...
            */
            ...
            rewards = ((((((balances * 100) / (totalStakedAmount / 10**18)) *
                (10**18)) +
                (timeFactor * (10**18)) +
    ...
    */
        ...
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < userStakeCount[_rewardAddress]; i++) {
            if (
                ...
                if (balances > 0) {
                    ...
                    rewards =
                        rewards +
                        ((((((balances * 100) / (totalStakedAmount / 10**18)) *
                            (10**18)) +
                            (timeFactor * (10**18)) +
                            apyFactorInWei) * balances) / 100);
                }
        ...
        if (claimedReward[_rewardAddress] > rewards) {
            ...
            totalReward =
                (rewards + oldReward[_rewardAddress]) -
                claimedReward[_rewardAddress];
        }
