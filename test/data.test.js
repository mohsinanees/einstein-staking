...
async function calculateReward(user, stakeWeeks, totalStakedBalance) {
    let result = (((user.stakedBalance / totalStakedBalance) * 100 + stakeWeeks + (APY / 52) * stakeWeeks) *
        user.stakedBalance) /
        100;
...
contract("PDOG_STAKING", async (accounts) => {
    before(async () => {
        ...
        for (let i = 0; i < 5; i++) {
            await pdog.transfer(accounts[i], (users[i].stakedBalance * 10 ** 18).toString());
            await pdog.approve(
                pdogstaking.address,
                (users[i].stakedBalance * 10 ** 18).toString(),
                {
                    from: accounts[i],
                }
            );
            await pdogstaking.stakeTokenForReward((users[i].stakedBalance * 10 ** 18).toString(), {
                from: accounts[i],
            });
        }
    });
